...
func NewProcessor() *Processor {
	processor := &Processor{modbus: &modbus.RtuHandler{}, Stg: storage.GenEtcdStorage(), Tw: wheelTimer.NewTimer(), Dt: store.NewTimerStore(), al: store.NewAlarmStore(), gu: store.NewGuidStore(), sl: store.NewSlaveStore(), workerPool: goroutine.Default()}
		...
		for event := range ch {
			...
			for i := range event.Events {
				...
				case storage.EventTypePut:
					...
					//key := event.Events[i].Key[len("device/"+guid):]
...
func (p *Processor) activeStore(action, guid, val string) error {
	...
	case "code":
		...
		if err != nil {
			log.Errorf("guid:%v device metadata transform error.", guid)
			return err
		}
...
func (p *Processor) register(data *model.RegisterData) error {
	...
	if err != nil {
		log.Warnf("guid:%v device metadata not found.", guid)
		return err
	}
	//3. 认证成功开始配置元数据信息
	if len(val) > 0 {
		...
		w, _ := p.Dt.Get(context.TODO(), guid)
		fmt.Println(w)
	}
	return nil
}
