...
func NewLine(cache *CacheStore, mqttBroker mqtt.Broker) {
	Cache = cache
...
func (t *LineTimer) Execute() {
	if address, err := Cache.Guid.Get(context.TODO(), t.Guid); err == nil && address != "" {
		var i int
		if t.Status > 0 { //之前是在线
			slaveList, err := Cache.Slave.Get(context.TODO(), address)
			if err != nil {
				return
			}
			...
			t.Status = 1 //改为上线
			slaveList, err := Cache.Slave.Get(context.TODO(), t.Guid)
			if err != nil {
				return
			}
			for _, slave := range slaveList {
